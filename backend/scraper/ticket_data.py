...
async def save_or_update_file(url, page, data, filename="ticket_data.json"):
    try:
        ...
        try:
            ...
            existing_data = {
                'url': url,
                'title': await page.locator('.sc-1eku3jf-14.ghwxrG').text_content(),
                'held_on': convert_date(await page.locator('.sc-1eku3jf-16.dCPMfd').text_content()),
                'location': await page.locator('.sc-1akkrr6-1.dvPJxG').text_content(),
                'sections': {},
                'recommendations': {}
            }
